...
npx esbuild server.ts --bundle --platform=node --format=esm --outfile=server-dist.js --minify --external:@prisma/client --external:".prisma/client" --external:vite --banner:js=`"import { createRequire } from 'module'; const require = createRequire(import.meta.url);`"
