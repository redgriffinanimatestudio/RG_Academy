...
# Version: 2.16 (Final Deep Stabilization)
# ==========================================================
...
Write-Host "🦾 STARTING INDUSTRIAL DEPLOYMENT PIPELINE v2.16" -ForegroundColor Cyan
Write-Host "===============================================" -ForegroundColor Cyan

# [1/6] Syncing Git
Write-Host "`n[1/6] 💾 Syncing Git..." -ForegroundColor Yellow
git add .
git commit -m "Auto-sync (v2.16 Fix): $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')" 2>$null || $true
git push origin main 2>$null || $true

# [2/6] Rebuild Assets
Write-Host "[2/6] 🏗️ Rebuilding Assets (v2.16 Hard Clean)..." -ForegroundColor Yellow
...
Write-Host "[4/6] 📦 Creating Archive (v2.16)..." -ForegroundColor Yellow
...
# [6/6] Finalize Remote
Write-Host "[6/6] ⚡ Finalizing v2.16 (Hotfix Sync)..." -ForegroundColor Yellow

$REMOTE_COMMANDS = @"
cd `$REMOTE_BASE || exit 1
(pkill -u `$SSH_USER node || true) && sleep 5
...
cd nodejs && `$NODE_PATH ./node_modules/.bin/prisma generate
mkdir -p tmp && touch tmp/restart.txt
cd .. && rm `$DEPLOY_ZIP `$SQL_DUMP
echo "--- DEPLOY v2.16 SUCCESS ---"
"@

ssh -p $SSH_PORT "$($SSH_USER)@$($SSH_HOST)" $REMOTE_COMMANDS

Write-Host "`n===============================================" -ForegroundColor Green
Write-Host "✅ DEPLOY v2.16 COMPLETED!" -ForegroundColor Green
