...
const RoleNode: React.FC<RoleNodeProps> = ({ title, icon, x, y, color, onShowDetail, id }) => (
  ...
  >
    <div 
      className="size-16 rounded-2xl bg-[#0f0f0f] border-2 flex items-center justify-center transition-all duration-500 shadow-xl group-hover:shadow-2xl"
      style={{ borderColor: color, boxShadow: `0 0 20px ${color}20` }}
    >
      <div style={{ color }}>{icon}</div>
      <div className="absolute -bottom-8 left-1/2 -translate-x-1/2 whitespace-nowrap text-[8px] font-black uppercase tracking-widest text-white/40 group-hover:text-white transition-colors">
        {title}
      </div>
    </div>
...
export const RoleTree: React.FC<{ onShowDetail: (id: string) => void }> = ({ onShowDetail }) => {
  return (
    <div className="relative w-full h-[400px] bg-black/40 rounded-[3rem] border border-white/5 overflow-hidden p-10">
      {/* Central Node */}
      <RoleNode 
        id="user" title="Node: User" icon={<User size={24} />} 
      ...
      <RoleNode 
        id="student" title="Academy Path" icon={<GraduationCap size={24} />} 
      ...
      <RoleNode 
        id="client" title="Studio Path" icon={<Briefcase size={24} />} 
      ...
      <RoleNode 
        id="executor" title="Pro Path" icon={<Users size={24} />} 
