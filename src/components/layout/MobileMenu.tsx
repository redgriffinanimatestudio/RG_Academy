...
import { X, Search, GraduationCap, Box, Users, ChevronDown, ChevronRight, LogIn, LogOut, Settings, Globe, Shield } from 'lucide-react';
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar p-6 space-y-8">
              ...
              <div className="relative group">
                ...
                <input 
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 pl-12 pr-4 text-[11px] font-bold uppercase tracking-widest outline-none focus:border-primary/40 focus:bg-white/[0.07] transition-all" 
                />
              </div>
              ...
              {/* Dynamic Sub-sections (Academy/Studio) */}
                ...
                {mobileSections.map(section => (
                  <div key={section.id} className="space-y-2">
                    <button 
                      ...
                      className={`w-full flex items-center justify-between p-4 rounded-2xl transition-all ${expandedSections[section.id] ? 'bg-white/5 border-white/10' : 'hover:bg-white/[0.02]'}`}
                    >
                      <div className="flex items-center gap-4">
                        <section.icon size={18} className={expandedSections[section.id] ? section.color : 'text-white/40'} />
                          ...
                          {section.categories.map((cat: any) => (
                            <div key={cat.name} className="py-2">
                              <div className="flex items-center gap-2 mb-2 opacity-40">
                                <cat.icon size={12} />
                                <span className="text-[9px] font-black uppercase tracking-widest">{t(cat.name)}</span>
                              </div>
                              <div className="grid grid-cols-1 gap-1">
                                {cat.subcategories.map((sub: any) => (
                                  <button 
                                    key={sub.name} 
                                    onClick={() => { handleSetSub(sub, cat); onClose(); }} 
                                    className={`px-3 py-2 text-[10px] font-bold uppercase text-left rounded-lg transition-colors ${activeSubName === sub.name ? section.color : 'text-white/30 hover:text-white hover:bg-white/5'}`}
                                  >
                                    {t(sub.name)}
                                  </button>
                                ))}
              ...
              <div className="flex gap-1 p-1 bg-black/40 rounded-2xl border border-white/5">
                {LANGUAGES.map(l => (
                  <button 
                    ...
                    className={`flex-1 py-3 rounded-xl text-[9px] font-black uppercase transition-all ${currentLangCode === l.code ? `${modeBg} text-bg-dark shadow-lg` : 'text-white/20 hover:text-white/40'}`}
                  >
                    {l.code}
                  </button>
                ))}
              ...
              {profile ? (
                <div className="space-y-4">
                  <div className="flex items-center gap-4 p-4 bg-white/5 rounded-2xl border border-white/10">
                    <div className={`size-12 rounded-xl overflow-hidden border border-white/10 shrink-0`}>
                      {profile.photoURL ? (
                        ...
                        <div className={`w-full h-full flex items-center justify-center text-lg font-black italic ${modeBg} text-bg-dark`}>
                    ...
                    <div className="flex flex-col min-w-0 flex-1">
                      ...
                      <span className={`text-[8px] font-bold uppercase tracking-widest ${modeColor}`}>{activeRole?.replace('_', ' ')}</span>
                    </div>
                    <button 
                      onClick={() => { logout(); onClose(); }} 
                      className="size-10 flex items-center justify-center text-rose-500 bg-rose-500/10 rounded-xl active:scale-90 transition-transform"
                    >
                      <LogOut size={18} />
                    </button>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <button onClick={() => { navigate(`/${currentLangCode}/dashboard`); onClose(); }} className="flex items-center justify-center gap-2 py-3 rounded-xl bg-white/5 text-[9px] font-black uppercase tracking-widest text-white/60 hover:text-white transition-colors">
                      <Shield size={14} /> Dashboard
                    </button>
                    <button className="flex items-center justify-center gap-2 py-3 rounded-xl bg-white/5 text-[9px] font-black uppercase tracking-widest text-white/60 hover:text-white transition-colors">
                ...
                <Link 
                  to={`/aca/${currentLangCode}/login`} 
                  onClick={onClose} 
                  className={`flex items-center justify-center gap-3 w-full py-5 ${modeBg} text-bg-dark text-xs font-black uppercase tracking-widest rounded-2xl shadow-xl active:scale-95 transition-all`}
                >
                  <LogIn size={18} /> Join Ecosystem
                </Link>
              )}
            </div>
          </motion.div>
