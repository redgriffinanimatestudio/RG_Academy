...
import { 
  ...
  Copy, Check, Send, Sparkles, Shield
} from 'lucide-react';
                  ...
                  <div className="w-full">
                    ...
                    {['student', 'lecturer', 'client', 'executor'].includes(activeRoleTab) && (
                      <div className="bg-white/5 rounded-[2rem] p-20 text-center space-y-6">
                        ...
                        <button 
                          onClick={() => {
                            navigate(`/aca/${lang || 'eng'}/dashboard`);
                          }}
          ...
          {activeTab === 'system' && (
            <motion.div key="system" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {[
                  { label: 'CPU Usage', val: '12%', status: 'Normal', icon: Activity, c: '#00f5d4' },
...
function Layers({ size }: { size: number }) { return <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="12 2 2 7 12 12 22 7 12 2"></polygon><polyline points="2 17 12 22 22 17"></polyline><polyline points="2 12 12 17 22 12"></polyline></svg>; }
