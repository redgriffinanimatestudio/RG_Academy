...
const CtaSection: React.FC<CtaSectionProps> = ({ t, user, lang, modePrefix, getDashboardLink, handleLoginRedirect }) => {
  return (
    <section className="max-w-5xl mx-auto px-4 text-center space-y-12">
      <h2 className="text-6xl md:text-8xl font-black tracking-tighter uppercase leading-[0.85] text-ink">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-6 w-full">
          {user ? (
            ...
            <button 
              ...
              className="bg-bg-card border border-border-main text-ink px-12 py-5 rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-emerald-500 hover:text-white transition-all w-full sm:w-auto flex items-center justify-center gap-4 shadow-2xl active:scale-95"
            >
              <img src="https://www.google.com/favicon.ico" alt="" className="w-5 h-4" />
              {t('continue_google')}
            </button>
          )}
          <Link to={`${modePrefix}/${lang || 'eng'}/community`} className="px-12 py-5 border-2 border-border-main text-ink font-black uppercase tracking-widest text-xs hover:bg-emerald-500/10 transition-all w-full sm:w-auto text-center">
            {t('view_community')}
          </Link>
        </div>
        
        <div className="flex items-center gap-6 opacity-40 group text-text-muted">
          <div className="h-[1px] w-12 bg-border-main" />
